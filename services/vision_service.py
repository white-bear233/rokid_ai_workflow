...
class VisionService:
    ...
    async def extract_keywords(
        ...
        system_prompt = (
            ...
            f"【任务】观察图片，提取用于网络搜索的关键词（3-6个词，空格分隔）\n\n"
            f"【重要规则】\n"
            f"1. **必须包含地理位置信息**：从用户位置中提取具体地名（如城市、区县、景点名称）作为关键词的一部分\n"
            f"2. 识别图片中的主要建筑/景点类型\n"
            f"3. 结合用户问题添加相关属性词（如历史、建造年代、文化等）\n"
            f"4. 只输出关键词，不要任何解释\n\n"
            f"【示例】\n"
            f"- 位置：黄山市 歙县 徽州古城，问题：建造年代 → 关键词：歙县徽州古城谯楼建造年代\n"
            f"- 位置：北京市东城区，问题：历史背景 → 关键词：北京故宫历史背景\n\n"
            f"请直接输出关键词："
        )
